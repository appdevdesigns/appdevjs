...
var findScripts = function(req, res, next){
	
    var moduleName = req.param('module');
    var directoryPath = __appdevPath + '/modules/' + moduleName + '/web/pages/'; 
    
    var list = [];
    
    var pages = AD.Util.FS.directoriesSync(directoryPath);
    
    for (var pageName in pages) {
    	
    	var pagesPath = directoryPath + '/' + pages[pageName] + '/tests/';
    	
        var runPath = '/'+ moduleName + '/' + pages[pageName] + '/tests/';
    	
    	var files = AD.Util.FS.filesSync(pagesPath);
    
    	for (var item in files) {
    		var obj = {
    			name:runPath.concat(files[item]),
    			id:runPath.concat(files[item])
    		};
    		list.push(obj);
    	}
    }
...
//these are our publicly available /site/api/[moduleName]/script  links;
var publicLinks = {
        findAll: { method:'GET', uri:'/[moduleName]/unitTests/script', params:{module: '[module]'}, type:'resource' }
...
var unitTestingStack = [
        ...
        findScripts     // get a list of all directories
...
siteUnitTesting.setup = function( app ) {
	...
	////---------------------------------------------------------------------
	app.all('/site/unitTests/script', unitTestingStack, function(req, res, next) {
	    ...
	    logDump(req, 'finished unitTesting/find_scripts ['+totalCount+']');
