...
function Page( opt ) {
    ...
    this.pathTest        = '';
    this.myListLocalTest = [];
    
    this.myListTest       = [];	   // list of all CSS files required by this Page (including sub Containers)
    this.myListTestLoaded = false;
    ...
    this.resources = AD.App.Page.defaultResources;
    
    this.pageStack        = null;  // {array} of fn() to call on a page load call 
...
Page.prototype.getTest = function( list ) 
{
    if (typeof list == 'undefined') {list = []};
    
    if (!this.myListTestLoaded) { 
    
        this.loadTest();
    
    } 
    
    for (var iL in this.myListTest) {
        list.push(this.myListTest[iL]);
    }
...
Page.prototype.loadTest = function( scriptPath ) 
{


    for (var wi =0; wi < this.myListLocalTest.length; wi++) {
    
        this.myListTest.push( this.myListLocalTest[wi]);
    ...
    for(var i in this.containers) {
        ...
        var curList = this.containers[i].getTest(list);
        for (var iCL in curList) {
            this.myListTest.push(curList[iCL]);
        }
        
    }
    
    this.myListTestLoaded = true;
}
 ...
 *
 *  @return {String}
 *    the name (directory name) of this page.
 */
...
//console.log('widgetKey['+widgetKey+'] item: '+widgetsList[ci]);	        
 ...
 *
 * Allows you to add JavaScript or CSS dependencies needed by the interface
...
Page.prototype.loadJavascripts = function( scriptPath ) 
{
    ...
    // define the page's shared url components:
    var urlVars = {
		moduleName:this.module.name(),
		pageName:this.name()
	}
    
    // options for our filesSync
    var fileOpts = {
    		listObj:this.myListJS,
    		onFile:function(fileName) {
    			// return filename in url format
    			urlVars.fileName = fileName;
    			return AD.App.Url.getUrl('urlPageScript', urlVars);
    		}
    };
//    this.myListJS = filesSync(this.pathScripts, fileOpts);
    AD.Util.FS.filesSync(this.pathScripts, fileOpts);
...
{
	if (this.pathLabels == '') this.pathLabels = '/page/'+this.module.name()+'/'+this.name();
...
Page.prototype.createRoutes = function() {
    ...
    var _self = this;
    var nameModule = this.module.name();
    ...
    var listResourceTypes = ['css', 'images'];
...
//    this.app.get( myPath + '/css/*', localFile);
    ...
    for (var lRT=0; lRT<listResourceTypes.length; lRT++) {
    	this.app.get('/'+nameModule+'/'+namePage+'/'+listResourceTypes[lRT]+'/*', localFile);
    }
    ...
    var initUrl = AD.App.Url.getUrl('urlPageInit',initParams);
    this.app.get(initUrl, initStack);
 ...
 //console.log(_self.listLoadedLabels);
    ...
    var labels = function(req, res, next) {
        ...
        req.aRAD.response.pathPage = _self.module.name()+'/'+_self.name()+'/';
    ...
    this.app.get('/labels/'+nameModule+'/'+namePage+'/labels.js', labelsStack);
    ...
    var page = function (req, res, next) {
        ...
        adObj.response.pathTemplate = _self.pathPage+'/views/'+_self.namePage+'.ejs';
        adObj.response.themePageStyle = _self.themePageStyle;  // 'default': default Template Style, 'empty':empty Template Style   
        
        var viewer = AD.Viewer.currentViewer(req);
        adObj.response.labels.langKey = viewer.languageKey;
        
        
        adObj.response.templateData = { title:_self.namePage, labels:req.aRAD.response.labels };
        ...
        adObj.response.pathSteal = '/init/'+nameModule+'/'+namePage;
        if (req.session.stealSession) {
            adObj.response.pathSteal = '/steal/'+req.session.stealSession.key+req.aRAD.response.pathSteal;
        }
        
        // specify what default resources should be included:
        req.aRAD.page.resources = _self.resources;
    ...
    this.app.get('/page/'+nameModule+'/'+namePage, pageStack);
    
    
    
    
    
    //// A page can serve up static content in it's /css, /scripts, /images folders:
    // map our url references to relative paths from our directory:
    var routes = {
    		'urlPageCSS'    : '/css/',
    		'urlPageImage'  : '/images/',
    		'urlPageScript' : '/scripts/',
    		'urlPageView'   : '/views/',
    		'urlPageTest'   : '/tests/'
    }
    var values = { moduleName:nameModule, pageName:namePage, fileName:'' };
    AD.App.Url.setupFileRoutes(this.app, routes, values, this.pathPage);
...
//	this.loadFilePaths(this.pathCSS, 'myListLocalCSS', onError, this.pathPage, relPath);
	...
	var fileOpts = {
    		listObj:this.myListLocalCSS,
    		onFile:function(fileName) {
    			// return filename in url format
    			urlVars.fileName = fileName;
    			return AD.App.Url.getUrl('urlPageCSS', urlVars);
    		}
    };
    AD.Util.FS.filesSync(this.pathCSS, fileOpts);
 ...
 * @param {Function} onError
 */
Page.prototype.loadPageTest = function( onError ) 
{
    if (this.pathTest == '') {
        this.pathTest = this.pathPage + '/tests';
    }
...
//	this.loadFilePaths(this.pathCSS, 'myListLocalCSS', onError, this.pathPage, relPath);
	...
	var fileOpts = {
    		listObj:this.myListLocalTest,
    		onFile:function(fileName) {
    			// return filename in url format
    			urlVars.fileName = fileName;
    			return AD.App.Url.getUrl('urlPageTest', urlVars);
    		}
    };
    AD.Util.FS.filesSync(this.pathTest, fileOpts);
